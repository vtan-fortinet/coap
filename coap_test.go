...
func TestInitOpts(tst *testing.T) {
	...
	oa.initOpts("-n")
	if oa.Short != "n" {
		tst.Error("failed parse short")
	}

	oa = oaItem{}
	oa.initOpts("--name")
	if oa.Short != "" || oa.Long != "name" {
		tst.Error("failed parse long")
	}

	oa = oaItem{}
	oa.initOpts("-n --name")
	if oa.Short != "n" || oa.Long != "name" {
...
func TestParseIDs1(tst *testing.T) {
	...
	if i != 0 {
		tst.Errorf("shoudl return 0: %v", i)
	}
	if m1.Fn != "abcd.txt" {
		tst.Errorf("Fn != abcd.txt: %v", *m1)
	}

	os.Args = []string{"cmd", "-u", "abc.com"}
	i, _ = ParseIDs([]ID{{m1, "this is m1"}, {m2, "this is m2"}})
	if i != 1 {
		tst.Errorf("shoudl return 1: %v", i)
	}
	if m2.Url != "abc.com" {
		tst.Errorf("Url != abc.com: %v", *m2)
	}
}
