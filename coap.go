...
func (oa *oaItem) initOpts(line string) string {
	...
	for _, opt := range opts {
		...
		case opt[:2] == "--":
			oa.Long = opt[2:]
			if oa.Vname == "" && !oa.IsBool {
				oa.Vname = strings.ToUpper(oa.Long)
			}
		case opt[:1] == "-":
			oa.Short = opt[1:2]
			if len(opt) > 2 {
...
func (oa *oaItem) initCans(line []byte) {
	...
	if err != nil {
		panic("Failed to process candicates," + err.Error())
	}
	oa.Cans = string(line)
...
func parseComplex(dat string) (c complex128, err string) {
	...
	if !strings.HasPrefix(r, "(") || !strings.HasSuffix(x, "i)") {
		return
	}
